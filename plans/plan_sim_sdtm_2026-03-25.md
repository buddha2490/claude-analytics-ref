...
cohort/
  ...
  sim_rs.R
  sim_sc.R
  output-data/         All XPT files written here (create if not present)
    dm.xpt
    ex.xpt
    ec.xpt
    cm.xpt
...
- [ ] **haven::write_xpt()**: write to `cohort/output-data/<domain>.xpt` in lowercase
...
Create `cohort/` directory and `cohort/output-data/` directory. Install packages: `haven`, `xportr`, `tidyverse`.
...
6. Write XPT via `haven::write_xpt(domain_df, path = "cohort/output-data/<domain>.xpt")`
...
- All 18 XPT files exist in `cohort/output-data/`
